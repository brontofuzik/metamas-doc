...
Jade\footnote{Jade Agent Development Framework --- \url{http://jade.tilab.com/}} is an agent platform and agent development framework in one.
...
% Player
The \texttt{Player} abstract class---one of Thespin4Jade three core classes---models a player\footnote{Actually, the \texttt{Player} class models a player \textit{type} and its instances model player \textit{tokens}.}.
...
% Future
The \texttt{Future<T>} class models a \textit{future}\footnote{Also called a \textit{promise} or \textit{IOU (I owe you)}.}---an object that acts as a proxy for a initially unknown result of a yet-to-be-completed computation.
