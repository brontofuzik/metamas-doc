...
Therefore, we considered only the most general free and open source agent platforms: Jade, Janus\footnote{http://www.janus-project.org/Home} and Jason\footnote{\url{http://jason.sourceforge.net/Jason/Jason.html}}.
Eventually, we chose Jade which, at the time of writing this thesis, appears to be the de facto standard implementation platform in the MAS research community.
...
A multi-agent system running on Jade (viewed as a platform) is basically an instantiation of Jade (viewed as a framework).
...
It is a module extending the Jade framework with classes that can be subclassed to define platform-specific models of organizations in a MAS running on the Jade platform.
...
\begin{itemize}
	\item \texttt{OneShotBehavioursSenderState} implementing a sender state that is also a Jade's one-shot\footnote{An simple behaviour in Jade that executes only once.} behaviour, and
	\item \texttt{FSMBehaviourSenderState} implementing a sender state that is also a Jade's FSM behaviour\footnote{A composite behaviour in Jade with sub-behaviour arranged into a finite-state machine.}.
\end{itemize}
...
Since organizations in Thespian4Jade are agentified\footnote{An agentified organization is an agent in its own right---it it autonomous and can interact (communicate) with other agents.}, the class is an extension of Jade's \texttt{Agent} class.
...
% Role
The \texttt{Role} abstract class---another one of Thespian4Jade's three core classes---models a role and its instances model the role's positions.
...
In Thespian4Jade, organizations and roles are agentified, and therefore can have knowledge about other agents (players in particular) the system.
...
For example, in Jade a behaviour is invoked\footnote{More precisely, scheduled for execution.} by calling the \texttt{addBehaviour(behaviour: Behaviour)} method of the \texttt{Agent} class.
...
If a behaviour is to be invoked synchronously from another behaviour in Jade, the calling behaviour has to \textit{include} the called behaviour as its sub-behaviour\footnote{For example, the calling FSM behaviour includes the called behaviour as one of its states.}.
...
% Player
The \texttt{Player} abstract class---the last one of Thespin4Jade's three core classes---models a player\footnote{Actually, the \texttt{Player} class models a player \textit{type}, and its instances model player \textit{tokens}.}.
...
In Thespian4Jade, players, being agents, can have knowledge about other agents (organizations and roles in particular) the system.
...
The \texttt{IObserver} and \texttt{IObervable} interfaces specify the \textit{Observer} design pattern as employed in Thespian4Jade.
